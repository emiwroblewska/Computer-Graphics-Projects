...
namespace CG_Project_4
{
    public static class EditionMethods
    {
        ...
        // ----------------------- Moving Shapes -----------------------
        public static void UpdatePolygon(Polygon s, bool antiAliasing, Rectangle clip)
        {
            if (s.FillColor != null)
                FillingAlgorithms.FillPolygon(s.Vertices, s, s.FillColor);
            else if (s.FillPattern != null)
                FillingAlgorithms.FillPolygon(s.Vertices, s, s.FillPattern);
            ...
            foreach (PixelPoint pp in s.Vertices)
            {
                if (s.Vertices.IndexOf(pp) == s.Vertices.Count - 1)
                {
                    ...
                    if (clip != null && s != clip)
                        ClippingAlgorithm.LiangBarsky(new PixelPoint(pp.X, pp.Y), new PixelPoint(s.Vertices[0].X, s.Vertices[0].Y), clip, s, antiAliasing);
                }
                else
                {
                    int idx = s.Vertices.IndexOf(pp);
                    if (antiAliasing == true)
                        s.AllPixels = s.AllPixels.Union(DrawingAlgorithms.GuptaSproull(pp.X, pp.Y, s.Vertices[idx + 1].X,
                        s.Vertices[idx + 1].Y, s.shapeColor, (double)s.GetThickness())).ToList();
                    else
                        s.AllPixels = s.AllPixels.Union(DrawingAlgorithms.lineDDA(pp.X, pp.Y, s.Vertices[idx + 1].X,
                        s.Vertices[idx + 1].Y, s.shapeColor, s.GetThickness())).ToList();
                    if (clip != null && s != clip)
                        ClippingAlgorithm.LiangBarsky(new PixelPoint(pp.X, pp.Y), new PixelPoint(s.Vertices[idx + 1].X, s.Vertices[idx + 1].Y), clip, s, antiAliasing);
                }
            }
        }

        public static void UpdateLine(Line s, bool antiAliasing, Rectangle clip)
        {
            if (antiAliasing == true)
            ...
            if (clip != null)
                ClippingAlgorithm.LiangBarsky(new PixelPoint(s.Start.X, s.Start.Y), new PixelPoint(s.End.X, s.End.Y), clip, s, antiAliasing);
        }

        public static void UpdateShapes(List<Shape> shapes, bool antiAliasing, Rectangle clip)
        {
            foreach (Shape shape in shapes)
            {
                ...
                if (shape is Polygon) UpdatePolygon(shape as Polygon, antiAliasing, clip);
                if (shape is Line) UpdateLine(shape as Line, antiAliasing, clip);
            }
        }
