...
namespace CG_Project_4
{
    public static class DrawingAlgorithms
    {
        ...
        public static List<PixelPoint> HorizontalLine(int x1, int y1, int x2, int y2, Colour color, double thickness)
        {
            ...
            int delta = 1;
            if (y2 - y1 < 0) delta = -1;
            CopyPixelsAA(result, xStart, yStart, thickness / 2, color, two_dx_invDenom, two_v_dx * invDenom, 0, delta);
            CopyPixelsAA(result, xEnd, yEnd, thickness / 2, color, two_dx_invDenom, two_v_dx * invDenom, 0, -delta);
            while (xStart < xEnd)
            {
                xStart += 1;
                xEnd -= 1;
